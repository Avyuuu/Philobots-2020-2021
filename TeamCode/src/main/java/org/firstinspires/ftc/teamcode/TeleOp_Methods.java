...
public class TeleOp_Methods extends TeleOp_Members {
    ...
    public void forward(double power, double distance) {
        ...
        //pidMovement.reset();
        //pidMovement.setSetpoint(counts); //all it does it put counts into m_setPoint so that it can be used in next method
        //pidMovement.setInputRange(0, counts);
        //pidMovement.setOutputRange(0, speed);
        //pidMovement.setTolerance(1);
        //pidMovement.enable();
        ...
        /*
        while (opModeIsActive() && !pidMovement.onTarget()) {
            double correction = pidDrive.performPID(getAngle());
            double movement = pidMovement.performPID(back_left.getCurrentPosition());
            back_left.setPower(  movement - correction);
            front_left.setPower( movement - correction);
            back_right.setPower( movement + correction);
            front_right.setPower(movement + correction);

            telemetry.addData("Angle: ", "%.2f", getAngle());
            telemetry.addData("correction: ", "%.2f", checkDirection());
            telemetry.addData("movement power: ", movement);
        ...
        //if (Math.abs(degrees) > 359) degrees = (int) Math.copySign(359, degrees);
        pidMovement.reset();
        pidMovement.setSetpoint(counts); //all it does it put degrees into m_setPoint so that it can be used in next method
        pidMovement.setInputRange(0, counts);
        ...
        // On right turn we have to get off zero first.
        while (opModeIsActive() && back_right.getCurrentPosition() == 0) {
            back_left.setPower(power);
            front_left.setPower(power);
            back_right.setPower(power);
            front_right.setPower(power);
        ...
        do {
            power = pidMovement.performPID(back_right.getCurrentPosition()); // power will be - on right turn.
            back_left.setPower(power);
            front_left.setPower(power);
            back_right.setPower(power);
            front_right.setPower(power);
            telemetry.addData("currentPosition", back_right.getCurrentPosition());
            telemetry.addData("currentPower", pidMovement.performPID());
            telemetry.update();
        } while (opModeIsActive() && !pidMovement.onTarget());
    ...
    public void automatedPowerShots()
    {
        ...
        strafeLeft(0.4,28.25); PIDrotate(0,0.8);
        shoot();
        PIDrotate(6.5,0.8);
        shoot();
        PIDrotate(-6.5,0.8 );
